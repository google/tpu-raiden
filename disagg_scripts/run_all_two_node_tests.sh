...
TESTS=(
  ...
  # ---- PULL mode (decode pulls from prefill) ----
  "PULL baseline (1 req, T=1, W=1)|MODE=pull NUM_REQUESTS=1 TRANSPORT_PARALLELISM=1 WORKER_PARALLELISM=1"
  "PULL transport-only (1 req, T=2, W=1)|MODE=pull NUM_REQUESTS=1 TRANSPORT_PARALLELISM=2 WORKER_PARALLELISM=1"
  "PULL worker-only (2 req, T=1, W=2)|MODE=pull NUM_REQUESTS=2 TRANSPORT_PARALLELISM=1 WORKER_PARALLELISM=2"
  "PULL both (4 req, T=2, W=2)|MODE=pull NUM_REQUESTS=4 TRANSPORT_PARALLELISM=2 WORKER_PARALLELISM=2"
  "PULL heavy (8 req, T=2, W=4)|MODE=pull NUM_REQUESTS=8 TRANSPORT_PARALLELISM=2 WORKER_PARALLELISM=4"
  "PULL dtype bf16 (4 req)|MODE=pull NUM_REQUESTS=4 TRANSPORT_PARALLELISM=2 WORKER_PARALLELISM=2 DTYPE=bf16"
  "PULL dtype fp8 (4 req)|MODE=pull NUM_REQUESTS=4 TRANSPORT_PARALLELISM=2 WORKER_PARALLELISM=2 DTYPE=fp8"
)
