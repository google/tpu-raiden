...
void KVCacheManagerWithTransfer::StartRead(
  ...
  const auto registration_deadline = DeadlineFromNow();
  {
    ...
    auto [it, inserted] = active_recv_entries_.emplace(uuid, std::move(entry));
    if (!inserted) {
      failed_recving_.insert(req_id);
  ...
  std::vector<uint64_t> timed_out_plan_uuids;
  {
    ...
    for (auto it = send_entries_.begin(); it != send_entries_.end();) {
      ...
      if (waiting_expired || transfer_expired) {
        ...
        timed_out_plan_uuids.push_back(uuid);
...
void KVCacheManagerWithTransfer::ProcessPullStream(
  ...
  std::string preaccept_error;
  {
    ...
    if (stopping_) {
      ...
      preaccept_error = "producer staging slot pool is exhausted";
    } else {
      Slot slot = AcquireSlotLocked();
  ...
  // A positive response means the producer irrevocably owns a staging slot.
  ControlResponseHeader response;
  response.magic = kResponseMagic;
  response.status = 0;
        ...
        [this, uuid = req.uuid, entry,
         ...
         dst_block_ids = std::move(dst_block_ids)]() {
          try {
            ...
            entry->failed = true;
            (void)TerminalizeSendEntryLocked(
                uuid, entry, "accepted send failed during startup",
                /*waiting_only=*/false);
          }
        });
  } catch (const std::exception& e) {
...
void KVCacheManagerWithTransfer::StartPushInternal(
  ...
  std::shared_ptr<SendEntry> entry;
  {
    ...
    if (entry->phase != SendEntry::Phase::kTransferring ||
        entry->slot_idx < 0 || entry->slot_idx >= num_slots_) {
      throw std::runtime_error(
          "accepted send does not own a reserved staging slot");
    }
    if (static_cast<int64_t>(src_block_ids.size()) > max_blocks_) {
      throw std::runtime_error("accepted send exceeds staging slot capacity");
    }
    const Slot& slot = all_slots_[entry->slot_idx];
    ...
    // Keep the slot pinned while D2H futures are being issued. Polling may
    // logically time out this transfer, but cannot recycle the slot until this
    // startup guard and all issued callbacks have drained.
