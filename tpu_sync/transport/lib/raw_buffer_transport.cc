...
constexpr int kMaxPushThreads = 32;
...
absl::Status RawBufferTransport::PushBuffers(
  ...
  const int num_threads = std::clamp(parallelism, 1, kMaxPushThreads);
  tpu_raiden::NumaThreadPool* pool = nullptr;
  {
    absl::MutexLock lock(push_pool_mu_);
    if (!push_pool_) {
      push_pool_ = std::make_unique<tpu_raiden::NumaThreadPool>(num_threads);
    }
    pool = push_pool_.get();
  }
