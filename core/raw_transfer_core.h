...
class PjRtCopyFuture {
 public:
  ...
  void OnReady(std::function<void(absl::Status)> cb) {
    if (futures_.empty()) {
      cb(absl::OkStatus());
      return;
    }
    xla::Future<> joined = xla::JoinFutures(futures_);
    struct KeepAliveState {
      std::vector<std::shared_ptr<RawBufferHolder>> c_api_holds;
      std::vector<std::shared_ptr<xla::CommonPjRtBuffer::ScopedHold>> holds;
      std::vector<std::shared_ptr<xla::PjRtBuffer::ExternalReference>>
          ext_holds;
      std::vector<std::shared_ptr<void>> user_holds;
    };
    auto state = std::make_shared<KeepAliveState>();
    state->c_api_holds = std::move(c_api_holds_);
    state->holds = std::move(holds_);
    state->ext_holds = std::move(ext_holds_);
    state->user_holds = std::move(user_holds_);

    joined.OnReady(
        [cb = std::move(cb), state](absl::Status status) { cb(status); });
    futures_.clear();
  }
