...
TPU Raiden is a high-performance Key-Value (KV) cache orchestration, multi-tiered memory management, and cross-node data transfer library purpose-built to accelerate large-scale Large Language Model (LLM) inference and Reinforcement Learning (RL) training loops on Cloud TPU infrastructure.
...
To address these bottlenecks, we are introducing TPU Raiden — an open-source, native C++ framework for high-performance data transfer and KV cache management on TPUs. Raiden functions as the low-level, zero-copy memory and DMA foundation executing directly beneath Python-based ML orchestrators. By mapping directly onto hardware-accelerated memory layers, TPU Raiden bridges HBM, Local System Host DRAM, and Remote Disaggregated TPU Nodes, unlocking bare-metal throughput and context capacity with minimal latency overhead.

## 💡 Key Architectural Features

*   Zero-Copy PJRT Memory Aliasing: Raiden accesses internal JAX arrays and PyTorch tensors by extracting native `PjRtBuffer` hardware descriptors. This bypasses intermediary CPU bounce-allocation, establishing true zero-copy H2D, D2H, and H2H (reshard) tensor transmission.
...
TPU Raiden is actively validated and optimized for the following accelerator generations:
...
#### Installing Bazel
To compile the `tpu_raiden` C++ extension binaries, you will need Bazel 8.6.0.
...
> [!NOTE]
> The pre-built `tpu_raiden` wheel will be available on PyPI to the public shortly.
...
If you'd like to try out Raiden and see it in action, please refer to the [`examples/`](examples/) directory. This folder contains a collection of hands-on scripts designed for users to interact with the library, including:
