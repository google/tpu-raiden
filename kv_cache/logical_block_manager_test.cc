...
TEST(LogicalBlockManagerTest, InitialState) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    EXPECT_EQ(manager.GetEntityId(i), std::nullopt);
  }
}

TEST(LogicalBlockManagerTest, BasicAllocation) {
  LogicalBlockManager manager(5);
  auto blocks_or = manager.Allocate(3, /*entity_id=*/100, /*lock=*/false);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    EXPECT_FALSE(manager.IsLocked(i));
    EXPECT_THAT(manager.GetEntityId(i), Optional(Eq(100)));
  }
}

TEST(LogicalBlockManagerTest, AllocationWithLocking) {
  LogicalBlockManager manager(5);
  auto blocks_or = manager.Allocate(2, /*entity_id=*/200, /*lock=*/true);
  ASSERT_TRUE(blocks_or.ok());
...
TEST(LogicalBlockManagerTest, LruEvictionOrder) {
  ...
  auto blocks3 = manager.Allocate(2, 30);
  ASSERT_TRUE(blocks3.ok());
  EXPECT_THAT(*blocks3, ElementsAre(0, 1));

  EXPECT_THAT(manager.GetEntityId(0), Optional(Eq(30)));
  EXPECT_THAT(manager.GetEntityId(1), Optional(Eq(30)));
  EXPECT_THAT(manager.GetEntityId(2), Optional(Eq(20)));
  EXPECT_THAT(manager.GetEntityId(3), Optional(Eq(20)));
}
...
TEST(LogicalBlockManagerTest, AccessUpdatesLruOrder) {
  ...
  EXPECT_THAT(manager.GetEntityId(0), Optional(Eq(10)));
  EXPECT_THAT(manager.GetEntityId(2), Optional(Eq(30)));
}
...
TEST(LogicalBlockManagerTest, LockedBlocksPreventEviction) {
  ...
  ASSERT_TRUE(manager.Allocate(2, 10, /*lock=*/true).ok());
  // Allocate 2 unlocked blocks to entity 20.
  ASSERT_TRUE(manager.Allocate(2, 20, /*lock=*/false).ok());

  // Requesting 3 blocks should fail since only 2 blocks are evictable.
  auto failed_or = manager.Allocate(3, 30);
...
TEST(LogicalBlockManagerTest, UnlockAllowsEviction) {
  ...
  auto blocks_or = manager.Allocate(3, 30);
...
TEST(LogicalBlockManagerTest, InvalidArguments) {
  ...
  EXPECT_FALSE(manager.Allocate(0, 10).ok());
  EXPECT_FALSE(manager.Allocate(-1, 10).ok());
  EXPECT_FALSE(manager.Allocate(6, 10).ok());
