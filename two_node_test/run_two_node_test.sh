...
ENV_PREFIX="source ${CONDA_SH} && conda activate ${CONDA_ENV} && cd ${TEST_DIR} && export PYTHONPATH=${TPU_RAIDEN}:\${PYTHONPATH:-} && export RAIDEN_STAGING_MODE=${RAIDEN_STAGING_MODE:-} && export RAIDEN_CONN_POOL=${RAIDEN_CONN_POOL:-1}"
