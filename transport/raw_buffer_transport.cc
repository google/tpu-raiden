...
absl::Status RawBufferTransport::ReadExact(int fd, void* buffer,
  ...
  while (remaining > 0) {
    ...
    if (bytes_read == 0) {
      return absl::OutOfRangeError("EOF / Socket closed");
    }
    ptr += bytes_read;
...
RawBufferTransport::RawBufferTransport(RawBufferTransportDelegate* delegate,
  ...
  if (!local_ip.empty()) {
    ...
    auto it = ip_to_numa.find(local_ip);
    if (it != ip_to_numa.end()) {
      numa_node_ = it->second;
      LOG(INFO) << "RawBufferTransport bound to local_ip " << local_ip
                << " mapped to NUMA node " << numa_node_;
    } else {
      LOG(WARNING)
          << "RawBufferTransport bound to local_ip " << local_ip
          << " could not be mapped to a NUMA node. Thread pinning disabled.";
    }
  }
...
absl::StatusOr<int> RawBufferTransport::ConnectToPeer(const std::string& peer) {
  ...
  for (rp = result; rp != nullptr; rp = rp->ai_next) {
    ...
    if (!local_ip_.empty()) {
      ...
      int bind_ret =
          getaddrinfo(local_ip_.c_str(), "0", &bind_hints, &bind_result);
      if (bind_ret == 0 && bind_result != nullptr) {
        if (bind(sock_fd, bind_result->ai_addr, bind_result->ai_addrlen) < 0) {
          VLOG(1) << "ConnectToPeer: Failed to bind source socket to "
                  << local_ip_ << ": " << std::strerror(errno);
        } else {
          VLOG(1) << "ConnectToPeer: Successfully bound source socket to "
                  << local_ip_;
        }
        freeaddrinfo(bind_result);
      } else {
        VLOG(1) << "ConnectToPeer: getaddrinfo failed for source bind IP "
                << local_ip_ << ": " << gai_strerror(bind_ret);
      }
    }
...
absl::Status RawBufferTransport::ProcessSingleRequest(int client_fd) {
  ...
  if (header.op == 5) {
    ...
    LOG(ERROR) << "[RAIDEN_DEBUG] Received unknown op: "
...
void RawBufferTransport::ConnectionWorker(int client_fd) {
  if (numa_node_ >= 0) {
    if (tpu_raiden::PinCurrentThreadToNumaNode(numa_node_) != 0) {
      LOG(WARNING) << "Failed to pin ConnectionWorker thread to NUMA node "
                   << numa_node_;
    }
  }
  while (!stopping_) {
...
void RawBufferTransport::ListenerLoop() {
  if (numa_node_ >= 0) {
    if (tpu_raiden::PinCurrentThreadToNumaNode(numa_node_) != 0) {
      LOG(WARNING) << "Failed to pin ListenerLoop thread to NUMA node "
                   << numa_node_;
    }
  }
  while (!stopping_) {
