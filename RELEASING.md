...
|---------|---------------------------|--------------------------------------------|---------|
| nightly | `X.Y.Z.devYYYYMMDDHHMMSS` | `Nightly Wheels` workflow (daily 08:00 UTC) | `pip install --pre <wheel>` (see below) |
| stable  | `X.Y.Z`                   | `Release Wheels` workflow (on `vX.Y.Z` tag) | `pip install <wheel>` (see below) |
...
Two wheels are published per version, one per framework. Drop `--pre` from
the commands below to get the latest stable release instead of the latest
nightly:

```bash
REGISTRY="https://us-python.pkg.dev/cloud-tpu-inference-test/tpu-raiden/simple/"

# JAX: self-contained, pulls the pinned jax/jaxlib/libtpu stack.
pip install --pre tpu_raiden_jax --extra-index-url "${REGISTRY}"
...
pip install --pre tpu_raiden_torch --extra-index-url "${REGISTRY}"
```

Stable releases additionally attach the wheels and bare `.so` files
(`_tpu_raiden_jax.so`, `_tpu_raiden_host.so`, `_tpu_raiden_torch.so`) to a
GitHub Release on the tag.
