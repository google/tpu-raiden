...
StartTransferRequest plans used by the Stage 1 local primitive test, then sends bytes
...
def _owned_block_ids(rank: int, num_sources: int,
                     num_pages: int) -> Tuple[List[int], List[int]]:
...
def _dest_peer(dest_host: str, dest_base_port: int, dests_per_group: int,
               group: int, dest_rank: int) -> str:
  ...
  for token in range(live_tokens):
    ...
    for head in (0, 1):
      ...
      payload[offset:offset +
              HEAD_PAIR_BYTES] = bytes([value]) * HEAD_PAIR_BYTES
  return bytes(payload)


def _expected_dest_payload(layer: int, page: int, live_tokens: int,
                           head: int) -> bytes:
  payload = bytearray([POISON]) * DST_SLOT_BYTES
  for token in range(live_tokens):
    offset = token * HEAD_PAIR_BYTES
    value = _pattern_byte(layer, page, token, head)
    payload[offset:offset + HEAD_PAIR_BYTES] = bytes([value]) * HEAD_PAIR_BYTES
  return bytes(payload)
...
def _sender_request(uuid: int, peers: Sequence[str], rank: int,
                    num_sources: int, num_pages: int, num_tokens: int) -> Any:
...
def _receiver_request(uuid: int, peers: Sequence[str], dest_rank: int,
                      num_sources: int, num_pages: int,
                      num_tokens: int) -> Any:
...
def _validate_args(args: argparse.Namespace, num_tokens: int,
                   num_pages: int) -> None:
...
def _control_request(host: str, port: int, payload: Dict[str, Any],
...
def _make_source_managers(args: argparse.Namespace,
                          num_pages: int) -> List[Any]:
  managers = []
  for rank in range(args.num_sources):
    managers.append(
        kv_cache_manager.KVCacheManager.create_host_only(
            num_layers=NUM_FA_LAYERS,
            num_shards=1,
            slice_byte_size=SRC_SLOT_BYTES,
            node_id=rank,
            local_port=args.source_base_port + rank,
            host_blocks=_host_blocks_for_rank(rank, args.num_sources,
                                              num_pages),
...
def _make_destination_managers(args: argparse.Namespace,
                               num_pages: int) -> List[Any]:
  managers = []
...
def _fill_sources(managers: Sequence[Any], num_sources: int, num_pages: int,
                  num_tokens: int) -> None:
  for rank, manager in enumerate(managers):
    for page in _owned_pages(rank, num_sources, num_pages):
      live_tokens = _tokens_in_page(page, num_tokens)
      for layer in range(NUM_FA_LAYERS):
        manager.write_block_bytes(layer, page // num_sources,
...
def _register_sender_plans(managers: Sequence[Any], args: argparse.Namespace,
                           num_pages: int, num_tokens: int, dest_host: str,
                           active_uuids: List[int]) -> None:
  for group in range(args.groups):
    uuid = args.uuid_base + group
    peers = [
        _dest_peer(dest_host, args.dest_base_port, args.dests_per_group, group,
                   dest_rank)
    ...
    for rank, manager in enumerate(managers):
      manager.register_active_plan(
          uuid,
          _sender_request(uuid, peers, rank, args.num_sources, num_pages,
                          num_tokens),
          is_sender=True,
      )
...
def _unregister_sender_plans(managers: Sequence[Any],
                             active_uuids: Sequence[int]) -> None:
  for manager in managers:
...
def _run_pushes(managers: Sequence[Any], args: argparse.Namespace,
                num_pages: int, dest_host: str) -> Tuple[float, List[Dict[str,
                                                                         Any]]]:
  results = []
  results_lock = threading.Lock()
  threads = []

  def push_one(group: int, rank: int, dest_rank: int, peer: str,
               src_ids: Sequence[int], dst_ids: Sequence[int]) -> None:
    uuid = args.uuid_base + group
  ...
  for group in range(args.groups):
    for rank in range(args.num_sources):
      ...
      if not src_ids:
        continue
      for dest_rank in range(args.dests_per_group):
        peer = _dest_peer(dest_host, args.dest_base_port,
                          args.dests_per_group, group, dest_rank)
  ...
  return elapsed_s, sorted(
      results,
      key=lambda item:
      (item["group"], item["source_rank"], item["dest_rank"]),
  )


def _payload_bytes(groups: int, dests_per_group: int, num_pages: int,
                   num_tokens: int) -> int:
  per_dest_layer = 0
...
def _print_source_report(elapsed_s: float, results: Sequence[Dict[str, Any]],
                         groups: int, dests_per_group: int, num_pages: int,
                         num_tokens: int, verify_response: Dict[str,
                                                                Any]) -> None:
...
class DestinationServer:
  ...
  def __init__(self, args: argparse.Namespace, num_tokens: int,
               num_pages: int):
    self._args = args
  ...
  def _request_config(self, request: Dict[str, Any]) -> Tuple[int, int, int, str]:
  ...
  def _reset(self, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    self._unregister_active()
    _poison_destinations(self._managers[:groups * self._args.dests_per_group],
                         num_pages)
    for group in range(groups):
      uuid = int(request.get("uuid_base", self._args.uuid_base)) + group
      peers = [
          _dest_peer(dest_host, self._args.dest_base_port,
                     self._args.dests_per_group, group, dest_rank)
          for dest_rank in range(self._args.dests_per_group)
      ]
      for dest_rank in range(self._args.dests_per_group):
        manager = self._managers[_manager_index(group, dest_rank,
                                                self._args.dests_per_group)]
        manager.register_active_plan(
            uuid,
            _receiver_request(uuid, peers, dest_rank, self._args.num_sources,
      ...
      for dest_rank in range(self._args.dests_per_group):
        ...
        for layer in range(NUM_FA_LAYERS):
          for page in range(num_pages):
            ...
            expected = _expected_dest_payload(layer, page, live_tokens,
                                              dest_rank)
            if actual == expected:
              continue
            mismatches += 1
            if len(examples) < 10:
              examples.append(
                  self._describe_mismatch(group, dest_rank, layer, page,
                                          live_tokens, actual, expected)
              )
    elapsed_s = time.perf_counter() - start
    return {
        "ok": mismatches == 0,
  ...
  def _describe_mismatch(self, group: int, dest_rank: int, layer: int,
                         page: int, live_tokens: int, actual: bytes,
                         expected: bytes) -> Dict[str, Any]:
    offset = next(
        (idx for idx, pair in enumerate(zip(actual, expected))
         if pair[0] != pair[1]),
...
def _run_source(args: argparse.Namespace) -> int:
  ...
  print(
      json.dumps({
          ...
          "source_ports": [
              args.source_base_port + rank for rank in range(args.num_sources)
          ],
          "groups": args.groups,
          "num_pages": num_pages,
          "num_tokens": num_tokens,
  ...
  for run_idx in range(args.repeat):
    ...
    try:
      _register_sender_plans(managers, args, num_pages, num_tokens, dest_host,
                             active_uuids)
      reset_response = _control_request(
          dest_host, args.control_port, {
              "cmd": "reset",
      ...
      _print_source_report(elapsed_s, results, args.groups,
                           args.dests_per_group, num_pages, num_tokens,
                           verify_response)
    finally:
  ...
  if args.shutdown_destination:
    with contextlib.suppress(Exception):
      _control_request(dest_host, args.control_port, {"cmd": "shutdown"},
                       args.control_timeout_s)
  return 0
...
def _parse_args(argv: Optional[Sequence[str]]) -> argparse.Namespace:
  ...
  parser.add_argument("--uuid_base",
                      type=_nonnegative_int,
                      default=DEFAULT_UUID_BASE)
