...
def _fill_source(manager, page: int, live_tokens: int) -> None:
  for layer in range(NUM_FA_LAYERS):
    manager.write_block_bytes(layer, SOURCE_BLOCK_ID,
...
def _sender_request(uuid: int, peer: str, *, head: int, live_tokens: int,
...
class ReshardStage1LocalPrimitiveTest(absltest.TestCase):
  ...
  def _new_source_manager(self):
    return kv_cache_manager.KVCacheManager.create_host_only(
  ...
  def test_size_mismatch_fails_without_modifying_destination(self):
    ...
    source.register_active_plan(
        UUID,
        _sender_request(UUID,
                        peer,
                        head=1,
                        live_tokens=8,
                        include_other_peer=False),
        is_sender=True,
    )
    ...
    self.assertEqual(dest.read_block_bytes(0, DEST_BLOCK_ID),
