...
#if defined(TPU_RAIDEN_ENABLE_TELEMETRY)
#include <cstdint>
#include <memory>
#include <string>
...
TEST(MetricsApiDisabledTest, ZeroOverheadNoOpsWhenDisabled) {
  RaidenMetricStore& store = RaidenMetricStore::GetGlobalMetricStore();
  EXPECT_FALSE(store.HasBackends());

  // No-op calls should compile cleanly and execute safely
  store.IncrementCounter(metric_names::kSentBytesTotal, {}, 1024);
  store.SetGauge("tpu_raiden_buffer_occupancy_bytes", {}, 512.0);
  store.ObserveHistogram("tpu_raiden_transfer_latency_seconds", {}, 0.05);
  EXPECT_EQ(store.GetTextSnapshot(), "");
}

#endif  // TPU_RAIDEN_ENABLE_TELEMETRY
