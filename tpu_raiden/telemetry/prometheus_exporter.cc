...
std::string FormatDouble(double val) {
  if (std::abs(val) >= 1e5 && val == std::floor(val)) {
    return absl::StrFormat("%.0f", val);
  }
...
void PrometheusExporter::IncrementCounter(absl::string_view name, uint64_t val,
  ...
  try {
    std::string formatted_labels = labels.empty() ? "" : FormatLabels(labels);
    MetricKey key{std::string(name), std::move(formatted_labels)};
    absl::WriterMutexLock lock(&mutex_);
    counters_[std::move(key)] += static_cast<double>(val);
  } catch (...) {
...
void PrometheusExporter::SetGauge(absl::string_view name, int64_t val,
  ...
  try {
    ...
    MetricKey key{std::string(name), std::move(formatted_labels)};
    absl::WriterMutexLock lock(&mutex_);
    gauges_[std::move(key)] = static_cast<double>(val);
  } catch (...) {
    // noexcept safety guarantee
  }
}
...
void PrometheusExporter::ObserveHistogram(absl::string_view name, double val,
  ...
  try {
    ...
    MetricKey key{std::string(name), std::move(formatted_labels)};
    absl::WriterMutexLock lock(&mutex_);
    auto& hist = histograms_[std::move(key)];
...
std::string PrometheusExporter::GetPrometheusTextSnapshot() {
  try {
    absl::flat_hash_map<MetricKey, double> counters_copy;
    absl::flat_hash_map<MetricKey, double> gauges_copy;
    absl::flat_hash_map<MetricKey, HistogramValue> histograms_copy;
    {
    ...
    // Group and export counter metrics (btree_map for deterministic ordering)
    absl::btree_map<std::string, std::vector<std::pair<MetricKey, double>>>
        grouped_counters;
    for (const auto& [key, val] : counters_copy) {
    ...
    for (const auto& [name, items] : grouped_counters) {
      ...
      for (const auto& [key, val] : items) {
        absl::StrAppend(&text, name, key.formatted_labels, " ",
                        FormatDouble(val), "\n");
      }
    }
    ...
    absl::btree_map<std::string, std::vector<std::pair<MetricKey, double>>>
        grouped_gauges;
    for (const auto& [key, val] : gauges_copy) {
    ...
    absl::btree_map<std::string,
        ...
        grouped_histograms;
    for (const auto& [key, val] : histograms_copy) {
    ...
    for (const auto& [name, items] : grouped_histograms) {
      ...
      for (const auto& [key, hist] : items) {
        ...
        absl::StrAppend(&text, name, "_sum", base_labels, " ",
...
std::shared_ptr<PrometheusExporter> GetGlobalPrometheusExporter() {
  static absl::NoDestructor<std::shared_ptr<PrometheusExporter>>
      global_exporter(std::make_shared<PrometheusExporter>());
