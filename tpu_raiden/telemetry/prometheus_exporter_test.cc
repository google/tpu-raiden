...
TEST(PrometheusExporterTest, RecordAndExportFormat) {
  ...
  EXPECT_THAT(output, HasSubstr("tpu_raiden_sent_bytes_total{interface=\"ICI\"} 1024"));
}

TEST(PrometheusExporterTest, MetricMetadataConstantsMapped) {
  EXPECT_EQ(metric_metadata::kSentBytesTotal.name, metric_names::kSentBytesTotal);
...
TEST(PrometheusExporterTest, ConcurrentMetricUpdates) {
  ...
  fibers.reserve(kNumThreads);

  for (int i = 0; i < kNumThreads; ++i) {
    fibers.push_back(std::make_unique<thread::Fiber>([&exporter]() {
  ...
  EXPECT_THAT(
      snapshot,
      HasSubstr(absl::StrCat("tpu_raiden_sent_bytes_total ",
