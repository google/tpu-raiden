...
std::string GetRaidenMetricsPrometheusText() {
  return RaidenMetricStore::GetGlobalMetricStore().ExportPrometheusText();
}
...
PYBIND11_MODULE(telemetry_pybind, m) {
  ...
  m.def("get_raiden_metrics_prometheus_text", &GetRaidenMetricsPrometheusText,
        "Get snapshot of Raiden metrics formatted as Prometheus exposition text.");

  m.def("clear_backends", []() {
    RaidenMetricStore::GetGlobalMetricStore().ClearBackends();
  }, "Clear all registered telemetry backends.");

  m.def("record_counter", [](const std::string& name, double value,
                             const std::vector<std::pair<std::string, std::string>>& labels) {
    std::vector<std::pair<absl::string_view, absl::string_view>> label_views;
    label_views.reserve(labels.size());
    for (const auto& kv : labels) {
      label_views.push_back({kv.first, kv.second});
    }
    RaidenMetricStore::GetGlobalMetricStore().RecordCounter(name, value, label_views);
  }, py::arg("name"), py::arg("value"), py::arg("labels") = std::vector<std::pair<std::string, std::string>>{});

  m.def("record_gauge", [](const std::string& name, double value,
                           const std::vector<std::pair<std::string, std::string>>& labels) {
    std::vector<std::pair<absl::string_view, absl::string_view>> label_views;
    label_views.reserve(labels.size());
    for (const auto& kv : labels) {
    ...
    RaidenMetricStore::GetGlobalMetricStore().RecordGauge(name, value, label_views);
  }, py::arg("name"), py::arg("value"), py::arg("labels") = std::vector<std::pair<std::string, std::string>>{});

  m.def("record_histogram", [](const std::string& name, double value,
                               const std::vector<std::pair<std::string, std::string>>& labels) {
    std::vector<std::pair<absl::string_view, absl::string_view>> label_views;
    label_views.reserve(labels.size());
    for (const auto& kv : labels) {
    ...
    RaidenMetricStore::GetGlobalMetricStore().RecordHistogram(name, value, label_views);
  }, py::arg("name"), py::arg("value"), py::arg("labels") = std::vector<std::pair<std::string, std::string>>{});
}
