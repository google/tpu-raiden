...
class TelemetryBindingTest(absltest.TestCase):
  ...
  def test_telemetry_enabled_attribute(self):
    if not telemetry_ext.IS_TELEMETRY_ENABLED:
      self.assertFalse(telemetry_ext.IS_TELEMETRY_ENABLED)
      self.assertEqual(telemetry_ext.get_raiden_metrics_prometheus_text(), "")
      with self.assertRaisesRegex(ValueError, "Telemetry is disabled in this build"):
        telemetry_ext.configure_telemetry(["prometheus"])
      return

    self.assertTrue(telemetry_ext.IS_TELEMETRY_ENABLED)

  def test_configure_telemetry_enable_prometheus(self):
    if not telemetry_ext.IS_TELEMETRY_ENABLED:
      self.skipTest("Telemetry disabled in this build")
    telemetry_ext.configure_telemetry(["prometheus"])
