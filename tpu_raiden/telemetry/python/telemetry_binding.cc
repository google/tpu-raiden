...
#if defined(TPU_RAIDEN_ENABLE_TELEMETRY)
  m.attr("IS_TELEMETRY_ENABLED") = true;
  m.def(
      "configure_telemetry",
...
#else
  m.attr("IS_TELEMETRY_ENABLED") = false;
  m.def(
      "configure_telemetry",
      [](const std::set<std::string>& backends) {
        if (!backends.empty()) {
          throw nb::value_error("Telemetry is disabled in this build.");
        }
      },
      nb::arg("backends"),
      "Configures active C++ telemetry backends (no-op stub when telemetry disabled).");

  m.def(
      "get_raiden_metrics_prometheus_text",
      []() -> std::string { return ""; },
      "Exports Prometheus text snapshot (no-op stub when telemetry disabled).");
#endif
}
