...
class RaidenBlockID:
  ...
  def __init__(
    ...
    if impl is not None:
      ...
      self._impl = _impl.RaidenBlockID(
          raiden_id._impl, host_block_id, hbm_block_id, status_val  # pylint: disable=protected-access
      )
  ...
  @property
  def hbm_block_id(self) -> int:
    return self._impl.hbm_block_id
  ...
  def __eq__(self, other: Any) -> bool:
    if not isinstance(other, RaidenBlockID):
      return False
    return (
        self.raiden_id == other.raiden_id
        and self.host_block_id == other.host_block_id
        and self.hbm_block_id == other.hbm_block_id
...
class KVCacheStore:
  ...
  def load(
      self, block_hashes: list[bytes], dst_hbm_block_ids: list[int]
  ) -> dict[bytes, Any]:
    """Initiates H2D load for the given block hashes to destination HBM block IDs."""
    return self._impl.load(block_hashes, dst_hbm_block_ids)

  def poll_load_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of H2D loads.
  ...
  def save(
      self, block_hashes: list[bytes], src_hbm_block_ids: list[int]
  ) -> dict[bytes, Any]:
    """Initiates local D2H save for the given block hashes from source HBM block IDs."""
    return self._impl.save(block_hashes, src_hbm_block_ids)

  def poll_save_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of D2H saves.
  ...
  def evict(self, block_hashes: list[bytes]) -> dict[bytes, Any]:
    """Initiates host memory eviction for the given block hashes."""
    return self._impl.evict(block_hashes)

  def poll_evict_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of evictions.

    Returns (done, failed, pending) block hashes.
    """
    return self._impl.poll_evict_status()
