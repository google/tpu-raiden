...
class KVCacheStoreE2ETest(parameterized.TestCase):
  ...
  def _run_remote_read_e2e_test(
    ...
    # multi-device single hosts. Real multi-host coverage of BOTH MULTI_NUMA=0
    # and =1 (plus multi-round read_remote + consumer-side Save) lives in
    # current_work/global_prefix_cache_0718/two_node_read_remote/.
    if enable_multi_numa and len(self.devices) > 4:
      self.skipTest(
          "Multi-NUMA E2E test is not supported on single-host shared device "
          f"configurations (devices={len(self.devices)}). Skipping. "
    ...
    # Verify REMOTE status + owner job_a + matching source host_block_id for all.
    ...
    #     Mixes read_remote + Save + Load on one store.
  ...
  def test_remote_read_then_consumer_save(self):
    # read_remote -> Load -> consumer-side Save of the fetched data.
    self._run_remote_read_e2e_test(
