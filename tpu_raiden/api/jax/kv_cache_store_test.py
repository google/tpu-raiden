...
def setUpModule():
  ...
  orchestrator_binary = os.path.abspath(
      os.path.join(test_dir, "..", "..", "kv_cache", "raiden_orchestrator_main")
  )
  ...
  if not os.path.exists(orchestrator_binary):
    # Fallback for bazel-bin if running from workspace root
    root_dir = os.path.abspath(os.path.join(test_dir, "..", "..", ".."))
    orchestrator_binary = os.path.join(
        root_dir, "bazel-bin", "tpu_raiden", "kv_cache", "raiden_orchestrator_main"
    )
...
class KVCacheStoreTest(absltest.TestCase):
  ...
  def test_queue_flow_embedded(self):
    ...
    for _, blocks in results:
      ...
      self.assertEqual(blocks[0].status, kv_cache_store.BlockStatus.HOST)
  ...
  def test_queue_flow_end_to_end(self):
    ...
    # Insert local blocks into receiver
    receiver_store.insert(
    ...
    for i, hash in enumerate(remote_hashes):
      ...
      self.assertEqual(len(blocks), 1)
      self.assertEqual(blocks[0].status, kv_cache_store.BlockStatus.HOST)
      self.assertEqual(blocks[0].host_block_id, allocated_blocks[i])
