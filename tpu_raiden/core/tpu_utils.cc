...
NicClassification ClassifyNic(const std::string& bdf, int mtu) {
  if (bdf.empty()) {
...
std::vector<HostNicAddress> GetLocalHostNicAddressesInternal(
  ...
  for (const auto& nic : nics) {
    ...
    std::cerr << "[TOPOLOGY_DISCOVERY] Detected interface: "
              << nic.interface_name << ", BDF: " << (bdf.empty() ? "None" : bdf)
              << ", MTU: " << mtu << ", NUMA: " << nic.numa_node
              << ", Classification: "
              << ClassificationToString(nic.classification) << std::endl;
  }
  return nics;
}
}  // namespace internal
