...
TEST_F(RaidenControllerTest, CreateFailsOnBufferCreationFailure) {
  auto controller_or = RaidenController::Create(
...
TEST_F(RaidenControllerTest, ExpectedWorkerCountTimesOut) {
  ...
  ASSERT_FALSE(controller_or.ok());
  EXPECT_EQ(controller_or.status().code(), absl::StatusCode::kDeadlineExceeded);
  EXPECT_THAT(controller_or.status().message(),
              testing::HasSubstr("expected 2 worker(s)"));
  EXPECT_THAT(controller_or.status().message(),
              testing::HasSubstr("got 1"));
}
