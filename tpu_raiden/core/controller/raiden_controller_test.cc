...
TEST_F(RaidenControllerTest, CreateWithServerAddressWorks) {
  EXPECT_EQ(test_server_->service->GetBufferCount(), 0);
  {
    ASSERT_OK_AND_ASSIGN(
        auto controller,
...
TEST_F(RaidenControllerTest, ExpectedWorkerCountTimesOut) {
  ...
  EXPECT_EQ(controller.status().code(), absl::StatusCode::kDeadlineExceeded);
  EXPECT_THAT(controller.status().message(),
              testing::HasSubstr("expected 2 worker(s)"));
  EXPECT_THAT(controller.status().message(), testing::HasSubstr("got 1"));
}
...
TEST_F(RaidenControllerTest,
  ...
  // After Create failed due to timeout, the callback capturing the
...
TEST_F(ReadRemotePullTest, FullSuccessPathUsesAuthoritativeIdsAndSrcEndpoints) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto dest, RaidenController::Create(
...
TEST_F(ReadRemotePullTest, HbmModeBuildsStagingPlusDeviceDst) {
  ...
  absl::Status st =
      dest->ReadRemote(src_raiden_id_, {10, 11}, {20, 21}, {"h0", "h1"},
...
TEST_F(ReadRemotePullTest, HostModeBuildsDramDstWithNoStaging) {
  ...
  ABSL_EXPECT_OK(dest->ReadRemote(src_raiden_id_, {10}, {20}, {"h0"}).Await());
  EXPECT_EQ(mock.vector_h2h_read_calls, 1);
