...
//     //tpu_raiden/core:raw_transfer_perf_test
...
void RunBenchmarkScenarioA(tpu_raiden::TpuPjrtManager* manager,
  ...
  auto dispatch_d2h = [&](const std::vector<xla::PjRtBuffer*>& bufs,
                          const std::vector<uint8_t*>& dsts,
                          const std::vector<size_t>& sizes) {
    int num_devices = devices.size();
    std::vector<std::vector<xla::PjRtBuffer*>> bufs_d(num_devices);
    std::vector<std::vector<uint8_t*>> dsts_d(num_devices);
    std::vector<std::vector<size_t>> sizes_d(num_devices);
    for (size_t i = 0; i < bufs.size(); ++i) {
      int d = i % num_devices;
      bufs_d[d].push_back(bufs[i]);
      dsts_d[d].push_back(dsts[i]);
      sizes_d[d].push_back(sizes[i]);
    }
  ...
  auto dispatch_h2d = [&](const std::vector<xla::PjRtBuffer*>& bufs,
                          const std::vector<const uint8_t*>& srcs,
                          const std::vector<size_t>& sizes) {
    int num_devices = devices.size();
    std::vector<std::vector<xla::PjRtBuffer*>> bufs_d(num_devices);
    std::vector<std::vector<const uint8_t*>> srcs_d(num_devices);
    std::vector<std::vector<size_t>> sizes_d(num_devices);
    for (size_t i = 0; i < bufs.size(); ++i) {
      int d = i % num_devices;
      bufs_d[d].push_back(bufs[i]);
      srcs_d[d].push_back(srcs[i]);
      sizes_d[d].push_back(sizes[i]);
    }
    std::vector<std::future<absl::Status>> futures;
    for (int d = 0; d < num_devices; ++d) {
  ...
  for (int iter = 0; iter < kWarmupIterations; ++iter) {
    dispatch_d2h(device_buffer_ptrs, host_dst_ptrs, host_sizes);
    dispatch_h2d(device_buffer_ptrs, host_src_ptrs, host_sizes);
  }
...
void RunBenchmarkScenarioB(tpu_raiden::TpuPjrtManager* manager,
  ...
  host_src_buffers.reserve(num_devices);
  ...
  for (int i = 0; i < num_devices; ++i) {
    ...
    ASSERT_OK(src_alloc_or.status());
    host_src_buffers.push_back(std::move(src_alloc_or).value());
    std::memset(host_src_buffers.back().ptr, 0, total_bytes);
    ...
    if (i < 2) {
      std::cout << "[INFO] host_src[" << i
                << "] ptr: " << static_cast<void*>(host_src_buffers.back().ptr)
                << ", actual NUMA node: "
                << GetPointerNumaNode(host_src_buffers.back().ptr) << std::endl;
  ...
  for (int i = 0; i < num_devices; ++i) {
    const uint8_t* src_bytes = host_src_ptrs[i];
