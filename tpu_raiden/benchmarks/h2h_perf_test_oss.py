...
_VERIFY = flags.DEFINE_bool('verify', False,
 'Best-effort data-integrity check on receiver (perf number '
 'is unaffected; dst block ids are allocated dynamically).')
...
def run_receiver():
 ...
 if _VERIFY.value:
 # best-effort: compare any non-zero blocks to their expected layer fill.
 # (dst block ids are allocated dynamically by h2h_write, so this is heuristic.)
 corrupt = 0
 for layer, d in enumerate(arrs):
 got = np.asarray(d)
 exp = _layer_fill(layer)
 nonzero = np.any(got != 0, axis=1)
 if np.any(nonzero) and not np.array_equal(got[nonzero], exp[nonzero]):
 corrupt += 1
 print(f'[receiver] verify: {corrupt} layer(s) mismatched'
 if corrupt else '[receiver] verify: OK (written blocks match).')
