...
def run_sender():
  ...
  # DEBUG: per-write logging so we see exactly which write hangs. If it stalls on
  # write #2+, the receiver ran out of dynamically-allocated blocks (pool
  # exhaustion). If it stalls on write #1, it's a first-transfer/protocol issue.
  _wn = [0]
  ...
  def one_write():
    ...
    print(f'[sender] write {i}: done in {dt*1e3:.1f} ms '
          f'(alloc={len(alloc)} dst blocks)', flush=True)
    return dt
