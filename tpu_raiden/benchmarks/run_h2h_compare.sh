...
#!/usr/bin/env bash
# Single-host cross-socket H2H: C++ (h2h_benchmark_runner) vs Python (push) side by
# side across the SAME setups. Run ON the TPU runner, inside the repo, after:
#   bazel build -c opt //examples/microbenchmarks:h2h_benchmark_runner \
...
CC=bazel-bin/examples/microbenchmarks/h2h_benchmark_runner
