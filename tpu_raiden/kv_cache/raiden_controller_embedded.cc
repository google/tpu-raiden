  ...
  while (!stopping_) {
    ...
    for (const auto& batch_item : req) {
      ...
      for (size_t i = 0; i < num_listeners_; ++i) {
        ...
        if (!start_transfer_i->shard_push_schedules().empty()) {
...
void RaidenControllerEmbedded::ConnectionWorker(int client_fd) {
  uint32_t net_len = 0;
  if (!transport::ReadExact(client_fd, &net_len, sizeof(net_len)).ok()) {
    close(client_fd);
    return;
  }
  ...
  if (!transport::ReadExact(client_fd, buffer.data(), payload_len).ok()) {
    close(client_fd);
    return;
  }
  ...
  if (resp.SerializeToString(&resp_str)) {
    ...
    transport::WriteExact(client_fd, &resp_net_len, sizeof(resp_net_len))
        .IgnoreError();
    transport::WriteExact(client_fd, resp_str.data(), resp_str.size())
        .IgnoreError();
  }
