...
class RoutingKVCacheManager final : public KVCacheManagerBase {
 public:
  ...
  absl::Status ReshardPush(const StartTransferRequest& request,
    ...
    route_.store(Route::kReshardPush, std::memory_order_release);
  ...
  absl::Status ReshardRegisterRecv(
    ...
    route_.store(Route::kReshardRegisterRecv, std::memory_order_release);
    return absl::OkStatus();
  }
...
ControlResponse SendRequest(int listener_port, const ControlRequest& request) {
  ...
  EXPECT_EQ(connect(sock, reinterpret_cast<sockaddr*>(&address),
                    sizeof(address)),
            0);
...
TEST(KVCacheListenerTest, PoolSenderRoutesToReshardPush) {
  ...
  EXPECT_EQ(manager.route(), Route::kReshardPush);
...
TEST(KVCacheListenerTest, PoolReceiverRoutesToReshardRegisterRecv) {
  RoutingKVCacheManager manager;
  KVCacheListener listener(&manager, /*listener_port=*/0);
  ...
  EXPECT_TRUE(response.success()) << response.message();
  EXPECT_EQ(manager.route(), Route::kReshardRegisterRecv);
  EXPECT_EQ(manager.request().uuid(), 1234);
...
TEST(KVCacheListenerTest, PoollessReceiverPreservesRegisterActivePlan) {
  ...
  // pool_dtype_tags predates the Stage-3 pool executor fields and must not by
  // itself opt a legacy command into ReshardRegisterRecv.
