...
#if __has_include("net/util/ports.h")
#include "net/util/ports.h"
#define HAVE_NET_UTIL_PORTS 1
#else
#include <unistd.h>
...
#ifndef EXPECT_OK
#define EXPECT_OK(status) EXPECT_THAT(status, ::absl_testing::IsOk())
#endif
#ifndef ASSERT_OK
#define ASSERT_OK(status) ASSERT_THAT(status, ::absl_testing::IsOk())
#endif
...
// Verifies E2E fetch flow with auto-allocated destination block ID.
TEST(RemoteFetchInteractionTest, QueueFlowEndToEndAutoAllocate) {
  // 1. Setup Orchestrator
  int orch_port = net_util::PickUnusedPortOrDie();
  RaidenOrchestrator orchestrator(orch_port);
  std::string orch_addr = absl::StrCat("localhost:", orch_port);
  ...
  size_t block_size = manager1.bytes_per_block();
  uint8_t* src_ptr = manager1.GetHostPointer(0, 0);
  std::memset(src_ptr + 1 * block_size, 'X', block_size);
  std::memset(src_ptr + 2 * block_size, 'Y', block_size);
  ...
  // 7. Lookup multiple hashes in store2 (should find them as REMOTE with
  // host_block_id = -1)
  ...
  // 8. Prepare InsertAndPin arguments (do NOT assign destination block IDs,
  // keep them -1)
  std::vector<std::string> pin_hashes;
  std::vector<RaidenBlockID> pin_slices;
  ...
  for (auto& pair : lookup_res) {
    pin_hashes.push_back(pair.first);
    EXPECT_EQ(pair.second.status, BlockStatus::REMOTE);
    EXPECT_EQ(pair.second.host_block_id, -1);
    pin_slices.push_back(pair.second);
  }
  ...
  // 10. Call Fetch for remote hashes
  ...
  // 12. Verify Destination Buffer and Status Upgrade in Store 2
  for (size_t i = 0; i < 2; ++i) {
    const auto& hash = lookup_hashes[i];
    auto lookup_res_after = store2.Lookup({hash});
    ASSERT_TRUE(lookup_res_after.ok());
    ASSERT_EQ(lookup_res_after->size(), 1);
    const auto& block_list = (*lookup_res_after)[0].second;
    EXPECT_EQ(block_list.status, BlockStatus::HOST);

    int allocated_block_id = block_list.host_block_id;
    EXPECT_GE(allocated_block_id, 0);
    EXPECT_LT(allocated_block_id, 10);

    uint8_t* dst_ptr = manager2.GetHostPointer(0, 0);
    char expected_char = (i == 0) ? 'X' : 'Y';
    for (size_t j = 0; j < block_size; ++j) {
      EXPECT_EQ(dst_ptr[allocated_block_id * block_size + j], expected_char)
          << "Mismatch for hash " << hash << " at block " << allocated_block_id
          << " index " << j;
    }
  }
