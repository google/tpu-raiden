...
TEST(RemoteFetchInteractionTest, QueueFlowEndToEnd) {
  ...
  // Use id1.job_name as host_address so resolution works
  std::vector<global_registry::Registration> regs = {
      {"remote_hash1", id1.job_name, 1},
      {"remote_hash2", id1.job_name, 2},
      {"remote_hash3", id1.job_name, 3},
      {"remote_hash4", id1.job_name, 4},
  };
...
TEST(RemoteFetchInteractionTest, QueueFlowMultiRemoteEndToEnd) {
  ...
  std::vector<global_registry::Registration> regs = {
      {"remote_hash1", id1.job_name, 1},
      {"remote_hash2", id1.job_name, 2},
      {"remote_hash3", id3.job_name, 3},
      {"remote_hash4", id3.job_name, 4},
  };
  ASSERT_OK(registry_client.Register(regs));
