...
TEST(KVCacheStoreTest, GlobalLookupFallback) {
  ...
  // Case 3: No local hit, only global hits
  {
    ...
    EXPECT_EQ((*lookup_res)[0].second[0].raiden_id, host1);

    EXPECT_EQ((*lookup_res)[1].first, "global_hash_2");
    EXPECT_EQ((*lookup_res)[1].second[0].raiden_id, host2);
  }
  ...
  // It should return both local and global
  {
    ...
    EXPECT_EQ((*lookup_res)[1].second[0].raiden_id, host1);

    EXPECT_EQ((*lookup_res)[2].first, "global_hash_2");
    EXPECT_EQ((*lookup_res)[2].second[0].raiden_id, host2);
  }
...
TEST(RemoteFetchInteractionTest, QueueFlowEmbedded) {
  ...
  std::vector<std::vector<RaidenBlockID>> slices1_1 = {
      {RaidenBlockID(id1, /*host_id=*/3, BlockStatus::HOST)}};
  store1.Insert({"hash1"}, slices1_1, /*on_host=*/true);
  std::vector<std::vector<RaidenBlockID>> slices1_2 = {
  ...
  std::vector<std::vector<RaidenBlockID>> slices2_1 = {{RaidenBlockID(
      id1, /*host_id=*/5, BlockStatus::REMOTE)}};  // Destination block 5
  store2.Insert({"hash1"}, slices2_1, /*on_host=*/true);
  std::vector<std::vector<RaidenBlockID>> slices2_2 = {{RaidenBlockID(
  ...
  ASSERT_EQ(block_list1.size(), 1);
  EXPECT_EQ(block_list1[0].status, BlockStatus::HOST);
  EXPECT_EQ(block_list1[0].host_block_id, 5);
  EXPECT_EQ(block_list1[0].raiden_id.job_name, "job2");
  ...
  ASSERT_EQ(block_list2.size(), 1);
  EXPECT_EQ(block_list2[0].status, BlockStatus::HOST);
  EXPECT_EQ(block_list2[0].host_block_id, 6);
  EXPECT_EQ(block_list2[0].raiden_id.job_name, "job2");
}
...
TEST(RemoteFetchInteractionTest, QueueFlowEndToEnd) {
  ...
  std::vector<std::vector<RaidenBlockID>> pin_slices;
  ...
  for (auto& pair : lookup_res) {
    ...
    if (pair.second[0].status == BlockStatus::REMOTE) {
      // Assign allocated local block ID
      pair.second[0].host_block_id = allocated_blocks[remote_count++];
    }
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    ASSERT_EQ(block_list.size(), 1);
    EXPECT_EQ(block_list[0].status, BlockStatus::HOST);
    EXPECT_EQ(block_list[0].host_block_id, allocated_blocks[i]);
  }

  // 15. Verify Async Write-Through in Global Registry
  // Wait for async write-through to complete by polling with short sleep.
  for (size_t i = 0; i < 4; ++i) {
...
TEST(RemoteFetchInteractionTest, QueueFlowMultiListenerEndToEnd) {
  ...
  // data is in managers.
  store1.Insert({"remote_hash1"},
                {{RaidenBlockID(id1, 1, BlockStatus::HOST),
                  RaidenBlockID(id1, 1, BlockStatus::HOST)}},
                true);
  ...
  if (pair.second[0].status == BlockStatus::REMOTE) {
    pair.second[0].host_block_id = allocated_blocks[0];
  }
  pin_slices.push_back(pair.second);
