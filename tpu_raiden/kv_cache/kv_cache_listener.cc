...
bool HasPoolReshardFields(
  ...
  // sends partially populated pool plans to ReshardPush/RegisterRecv so their
  // validation fails closed instead of silently taking the legacy path.
    ...
    if (req.has_start_transfer_request()) {
      ...
      if (is_pool_reshard && start_req.is_sender()) {
        ...
        const std::vector<int64_t> src_block_ids(
            start_req.src_block_ids().begin(),
            start_req.src_block_ids().end());
        absl::Status status = engine_->ReshardPush(
            start_req, src_block_ids, start_req.parallelism());
        if (!status.ok()) {
          resp.set_success(false);
          resp.set_message(std::string(status.message()));
          LOG(ERROR) << "ReshardPush native execution failed: " << status;
        }
        ...
        absl::Status status =
            engine_->ReshardRegisterRecv(start_req, chip_block_ids);
        if (!status.ok()) {
          resp.set_success(false);
          resp.set_message(std::string(status.message()));
          LOG(ERROR) << "ReshardRegisterRecv native execution failed: "
                     << status;
        }
