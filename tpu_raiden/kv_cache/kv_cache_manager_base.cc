...
// Collapse runs of adjacent copies into one. Consecutive entries whose src and
// dst both continue where the previous copy ended describe one contiguous
// region and merge into a single larger copy, so a run of N size-1 block copies
// becomes one N-block copy.
  ...
  if (total_works <= 1) {
    ...
    for (const auto& [node, works] : grouped_work) {
      ...
      auto status_or_local_futures = DispatchD2hWork(
          works, slot_idx, is_partial, src_c, dst_c, sizes_c);
      if (!status_or_local_futures.ok()) {
    ...
    for (const auto& [node, works] : grouped_work) {
      VLOG(1) << "DispatchD2hChunks: Scheduling dispatch for NUMA node " << node
              << ", works count: " << works.size();
      auto future = dma_pool_->Schedule(
          assigned_numa_node(), [this, works, is_partial, src_c, dst_c, sizes_c,
                                 slot_idx]() {
