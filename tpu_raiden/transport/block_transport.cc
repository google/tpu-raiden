...
absl::Status BlockTransport::HandleIncomingPull(
  ...
  const std::string s = lib::SerializeChunkHeader(resp_header);
...
void BlockTransport::H2hWriteWorker(int stream_idx, absl::string_view peer,
  ...
  header.uuid = uuid;
  const std::string s_header = lib::SerializeChunkHeader(header);
...
void BlockTransport::H2hReadWorker(
  ...
  for (const auto& chunk : chunks) {
    ...
    absl::StatusOr<lib::ChunkHeader> d = lib::DeserializeChunkHeader(
