...
absl::Status RawBufferTransport::ProcessPeerRequest(int client_fd) {
  ...
  if ABSL_PREDICT_FALSE (header.op == kOpBufferPull) {  // peer pull request
    ...
    for (uint32_t i = 0; i < batch_size; ++i) {
      absl::string_view item_bytes(meta_buf.data() + i * m_size, m_size);
...
absl::Status RawBufferTransport::PullBuffer(
  ...
  const std::string s_header = SerializeChunkHeader(header);
  RETURN_IF_ERROR(
      WriteExact(fd, s_header.data(), s_header.size()));
...
absl::Status RawBufferTransport::PushBuffer(absl::string_view peer,
  ...
  const std::array<struct iovec, 2> iovs = {
      iovec(const_cast<char*>(s_header.data()),
            s_header.size()),
...
absl::Status RawBufferTransport::PushBatch(
  ...
  header.metadata_size =
      static_cast<uint16_t>(GetChunkMetadataSize(header.version));
  header.remote_id = 0;
  header.local_id = 0;
  ...
  serialized_metadata.reserve(header.metadata_size * batch_size);
