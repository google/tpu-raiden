...
bool PollUntilDone(peregrine::Transport* transport, peregrine::Handle handle,
  ...
  while (true) {
    const auto status_or_s = transport->Poll(handle);
...
TEST(SocketTransportTest, PointToPointWriteTransfer) {
  ...
  constexpr int port1 = 23456;
  constexpr int port2 = 23457;
  ...
  ASSERT_THAT(dst_payload, Pointwise(Ne(), src_payload));
  ...
  const peregrine::Request request = {
      ...
      .laddr = reinterpret_cast<uint8_t*>(src_payload.data()),
      .raddr = dst_payload.data(),
      .len = src_payload.size(),
  };
  // Post write transfer from transport1 to transport2 endpoint string.
  const std::string peer2 = absl::StrCat("127.0.0.1:", port2);
  const auto status_or_handle = transport1->Post(peer2, {request});
...
TEST(SocketTransportTest, PointToPointReadTransfer) {
  ...
  const peregrine::Request request = {
      .op = peregrine::Op::kRead,
      .laddr = local_payload.data(),
      .raddr = reinterpret_cast<uint8_t*>(remote_payload.data()),
      .len = remote_payload.size(),
  };
  const std::string peer2 = absl::StrCat("127.0.0.1:", port2);
  const auto status_or_handle = transport1->Post(peer2, {request});
  ASSERT_TRUE(status_or_handle.ok()) << status_or_handle.status().message();
  ...
  EXPECT_THAT(local_payload, Pointwise(Eq(), remote_payload));
}

TEST(SocketTransportTest, PointToPointIPv6WriteTransfer) {
  constexpr int port1 = 23460;
  constexpr int port2 = 23461;
  std::string src_payload =
  ...
  const std::string peer2 = absl::StrCat("[::1]:", port2);
  const auto status_or_handle = transport1->Post(peer2, {request});
  ASSERT_TRUE(status_or_handle.ok()) << status_or_handle.status().message();
...
TEST(SocketTransportTest, PointToPointIPv6ReadTransfer) {
  constexpr int port1 = 23462;
  constexpr int port2 = 23463;
