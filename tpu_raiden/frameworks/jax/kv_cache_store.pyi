...
class KVCacheStore:
  ...
  def release_and_delete(
      self,
      block_hashes: list[bytes],
      pending_evict_entries: list[tuple[bytes, RaidenBlockID]]
      | None = ...,
  ...
  def poll_fetch_remote_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    ...
    ...
  def load(self, block_hashes: list[bytes], dst_hbm_block_ids: list[int]) -> dict[bytes, LoadFuture]:
    """Initiates local H2D load for the given block hashes to destination HBM block IDs."""
    ...
  def poll_load_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of H2D loads. Returns (done, failed, pending) block hashes."""
    ...
  def save(self, block_hashes: list[bytes], src_hbm_block_ids: list[int]) -> dict[bytes, SaveFuture]:
  ...
  def poll_save_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of D2H saves. Returns (done, failed, pending) block hashes."""
    ...
  def evict(self, block_hashes: list[bytes]) -> dict[bytes, EvictFuture]:
  ...
  def poll_evict_status(self) -> tuple[list[bytes], list[bytes], list[bytes]]:
    """Polls the status of evictions. Returns (done, failed, pending) block hashes."""
    ...
