...
def make_registered_shard_index(mesh):
...
def init_weight_synchronizer_and_d2h(
  ...
  def _local_init_and_d2h(anchor, s_idx, l_idx):
    ...
    return jax.ffi.ffi_call(
        ...
        num_layers=np.int32(num_layers),
        listener_port=np.int32(listener_port),
        registered_shard_count=np.int32(mesh.size),
    )

  if registered_shard_index is None:
    registered_shard_index = make_registered_shard_index(mesh)
  ...
  return jax.shard_map(
      ...
      out_specs=out_spec,
  )(device_array, shard_idx, registered_shard_index)
...
def h2d(device_array, shard_idx, mesh, registered_shard_index=None) -> jax.Array:
  ...
  def _local_h2d(anchor, s_idx, l_idx):
    return jax.ffi.ffi_call(
        ...
        has_side_effect=True,
    )(anchor, s_idx, l_idx, registered_shard_count=np.int32(mesh.size))
