...
"""Build rules for TPU Raiden PyTorch framework integration.

This module provides wrappers around standard Bazel rules that dynamically
disable PyTorch targets when PyTorch support is disabled.
"""
...
load("@rules_python//python:defs.bzl", py_library_rule = "py_library", py_test_rule = "py_test")

def raiden_torch_cc_library(name, srcs = [], hdrs = [], deps = [], **kwargs):
    cc_library_rule(
        name = name,
        srcs = select({
            "//:torch_disabled": [],
...
def raiden_torch_nanobind_extension(name, srcs = [], deps = [], **kwargs):
...
def raiden_torch_cc_test(name, srcs = [], deps = [], **kwargs):
    ...
    cc_test_rule(
        name = name,
        srcs = select({
            "//:torch_disabled": [":" + name + "_dummy_main.cc"],
            "//conditions:default": srcs,
        }),
        deps = select({
            "//:torch_disabled": [],
...
def raiden_torch_py_library(name, srcs = [], deps = [], **kwargs):
    py_library_rule(
        name = name,
        srcs = select({
            "//:torch_disabled": [],
...
def raiden_torch_py_test(name, srcs = [], deps = [], **kwargs):
    """Wrapper around py_test that is disabled when PyTorch is disabled.
    ...
    kwargs["main"] = select({
        "//:torch_disabled": ":" + dummy_main_file,
