...
def _interleave_spans(rank, *, num_tokens, interleave, parallelism=8,
                      page_tokens=4096):
...
class RaidenControllerTest(absltest.TestCase):
  ...
  def test_span_declarations_fail_closed_at_registration(self):
    controller, _, src_units, _, _ = self._stage3_fixture(
        register_blocks=False
    )
    unit = src_units[0]
  ...
  def test_pool_tag_selection_is_request_data(self):
    ...
    with self.assertRaisesRegex(
        ValueError, "do not match any registered pool"
    ):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
      for generation in range(25):
        ...
        def lookup():
          ...
          try:
            return _lookup_ids(controller, req_id, uuid, [unit])
