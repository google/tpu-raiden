...
absl::Status SendRpcSync(absl::string_view address,
  ...
  absl::Status status = transport::WriteExact(fd, &net_len, sizeof(net_len));
  if (!status.ok()) {
    close(fd);
    return status;
  }

  status = transport::WriteExact(fd, payload.data(), payload.size());
  if (!status.ok()) {
    close(fd);
    return status;
  }
  ...
  status = transport::ReadExact(fd, &resp_net_len, sizeof(resp_net_len));
  if (!status.ok()) {
    close(fd);
    return status;
  }
  ...
  status = transport::ReadExact(fd, resp_buffer.data(), resp_len);
  if (!status.ok()) {
    close(fd);
    return status;
  }
